...
const ResetPassword = () => {
  ...
  const handleVerify = () => {
   const otp =   input.Ref.current.map((input)=>input.value).join("");
   if(otp.length != 6){
  ...
  const onSubmitNewPassword = async (e) => {
    e.preventDefaults();
    try{
      const response = axios.post(backendURL + "/reset-password",{email,otp,newPassword});
      if(response.status === 200 ){
  ...
  return (
      ...
      >
        <Link to ="/" className='position-absolute top-0 start-0 p-4 d-flex align-items-center gap-2 text-decoration-none'>
          ...
          <span className='fs-4 fw-semibold text-light'>Authicator</span>
        </Link>
        ...
        {!isEmailSent && (
          <div className="rounded-4 p-5 text-center bg-white" style ={{width:"100%", maxWidth: "400px"}}>
                ...
                <form onSubmit={onSubmitEmail}>
                  <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                    ...
                    <input type="email"  className="form-control bg-transparent border-0 ps-1 pe-4 rounded-end" placeholder='Enter Your Email address' style={{height:'50px'}} onChange={(e)=>setEmail(e.target.value)}  value={email}  required />
                  </div>
         ...
         <div className="d-flex justify-content-between gap-2 mb-4 text-center text-white-50 mb-2">
             {[...Array(6)].map((_,i)=> (
               ...
               <button className="btn btn-primary fw-semibold w-100" disabled={loading} onClick={handleVerify} >
         ...
         {/*New password form */}
         {isOtpSubmitted && isEmailSent && (
                <div className="rounded-4 p-4 text-center bg-white" style={{width:"900%",maxWidth:"500px"}}>
                  <h4>New Password</h4>
